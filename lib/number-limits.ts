    ...
    const fallbackImplementation = async () => {
      ...
      for (const limit of limits) {
        if (isNumberInRange(numberToCheck, limit.number_range)) {
          ...
          // Si max_times es 0, significa que no hay límite establecido
          const hasNoLimit = maxTimes === 0
          // Si no hay límite, siempre está disponible, de lo contrario verificar si hay suficientes tiempos
          const available = hasNoLimit ? true : remaining >= timesToSell
