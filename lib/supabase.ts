...
export function initConnectionMonitoring() {
  ...
  // Monitorear errores de WebSocket sin reemplazar console.error directamente
  // Usamos un enfoque más seguro para evitar bucles infinitos con otros componentes
  // que también reemplazan console.error (como client-layout.tsx)
  const monitorWebSocketErrors = function(message: string) {
    ...
    if (
      ...
      log(LogLevel.WARN, "Detectado error de conexión WebSocket", { error: message });
  ...
  // Configurar un observador para errores de WebSocket sin reemplazar console.error
  // Esto evita conflictos con otros componentes que también modifican console.error
  const originalErrorHandler = window.onerror;
  window.onerror = function(message, source, lineno, colno, error) {
    // Convertir a string si es necesario
    const errorMessage = message?.toString() || '';
    
    // Intentar manejar errores de WebSocket
    if (monitorWebSocketErrors(errorMessage)) {
      // Si ya lo manejamos, podemos evitar propagación adicional
      // pero permitimos que el manejador original también se ejecute
    }
    ...
    // Devolver false para permitir el comportamiento predeterminado
    return false;
  };
