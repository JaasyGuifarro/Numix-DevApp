...
// Configuración
const SYNC_QUEUE_KEY = "numix_sync_queue"
...
export class EnhancedSyncManager {
  ...
  public async syncPendingOperations(): Promise<void> {
    if (!this.isOnline || this.syncInProgress || this.pendingOperations.length === 0) return

    this.syncInProgress = true
    this.updateSyncStatus("pending")

    try {
      // Verificar conexión a Supabase antes de intentar sincronizar
      const { checkRealtimeConnection, switchToNextServer } = await import('./supabase')
      const isConnected = await checkRealtimeConnection()
      
      if (!isConnected) {
        // Intentar cambiar a un servidor alternativo
        const switchSuccess = await switchToNextServer()
        if (!switchSuccess) {
          // Si no se pudo cambiar de servidor, marcar como error y salir
